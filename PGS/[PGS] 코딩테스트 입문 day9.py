...
def solution(hp):
    n_general = hp // 5
    n_soldier = (hp - n_general * 5) // 3
    n_ergate = hp - n_general * 5 - n_soldier * 3
    return n_general + n_soldier + n_ergate
...
# 가위 바위 보 (+1)
# 문제 설명 : 가위는 2 바위는 0 보는 5로 표현합니다. 가위 바위 보를 내는 순서대로 나타낸 문자열 rsp가 매개변수로 주어질 때, 
...
# 구슬을 나누는 경우의 수 (+14)
# 문제 설명 : 머쓱이는 구슬을 친구들에게 나누어주려고 합니다. 구슬은 모두 다르게 생겼습니다. 머쓱이가 갖고 있는 구슬의 개수 balls와 
